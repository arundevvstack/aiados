...
// Must be executed securely server-side with Service Role Key
const supabaseUrl = process.env.VITE_SUPABASE_URL || 'https://pyzwkvcbfyssxobzeuml.supabase.co';
const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!serviceRoleKey) {
  console.error('FATAL: SUPABASE_SERVICE_ROLE_KEY is required to run the validation suite.');
  process.exit(1);
}
...
// We also need the anon client to test RLS and session login
const anonKey = process.env.VITE_SUPABASE_ANON_KEY || 'sb_publishable_Z-7h5-hxPvMC3UURhPRxjQ_NPabsBSe';
